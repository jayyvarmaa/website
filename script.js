...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isMobile) {
        ...
        if (devModeCounter) devModeCounter.classList.remove('hidden');
        return;
    }
    ...
    if (warningScreen) {
        ...
        void warningScreen.offsetHeight;
    ...
    // Hide developer mode counter initially
    if (devModeCounter) {
        devModeCounter.classList.remove('visible');
    }
    ...
    function handleKeyPress(e) {
        if (e.key.toLowerCase() === 'e' && warningScreen && warningScreen.style.opacity === '1') {
            ...
            // After fade out, hide it completely and show the terminal
            setTimeout(() => {
                warningScreen.style.display = 'none';
                if (terminal) terminal.classList.remove('hidden');
                if (topNav) topNav.classList.remove('hidden');
                
                // Show developer mode counter after a short delay
                if (devModeCounter) {
                    setTimeout(() => {
                        devModeCounter.classList.add('visible');
                    }, 500);
                }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nameElement && counterElement && counterText) {
        // Make sure the counter is initially hidden
        counterElement.classList.remove('visible');
        
        nameElement.addEventListener('click', function() {
            // Only proceed if the counter is visible
            if (!counterElement.classList.contains('visible')) return;
            ...
            // If the user clicked 7 times, reload the page after a short delay
            if (clickCount >= 7) {
                setTimeout(() => {
                    location.reload();
                }, 1000);
